...
for fname in filenames:
    ...
    hooks = [
        ...
        #     lambda self, input, output: dec_attn_weights.append(output[1])
        # ),
        # model.transformer.decoder.layers[3].multihead_attn.register_forward_hook(
        #     lambda self, input, output: dec_attn_weights.append(output[1])
        # ),
        # model.transformer.decoder.layers[4].multihead_attn.register_forward_hook(
        #     lambda self, input, output: dec_attn_weights.append(output[1])
        # ),
    ...
    fig, axs = plt.subplots(ncols=target.shape[0]+3, nrows=1, squeeze=False, figsize=(5*(3+target.shape[0]), 5))
    colors = COLORS * 100
    ...
    for row in range(1):
        for col in range(3+target.shape[0]):
            ...
            if col == 0:
                ...
                ax.imshow(sattn[:,:, int(target.numpy()[col-3,1]*h), int(w*target.numpy()[col-3,0])].view(h, w),cmap='cividis', interpolation='nearest')
